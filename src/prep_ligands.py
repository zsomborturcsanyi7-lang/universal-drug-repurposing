...
def smiles_to_3d_pdb(smiles: str, name: str, output_dir: str = "ligands") -> str:
    ...
    # Flavor 32 writes CONECT records which helps OpenBabel detect bonds
    # Flavor 2 writes additional info
    Chem.MolToPDBFile(mol, pdb_path, flavor=32)
...
def pdb_to_pdbqt(pdb_path: str, output_path: str) -> bool:
    ...
    try:
        ...
        # Vina 1.2.x is very strict. Check if ROOT exists.
        # If not, it's a malformed PDBQT for Vina.
        if os.path.exists(output_path):
            with open(output_path, 'r') as f:
                content = f.read()
                if "ROOT" not in content:
                    logger.warning(f"Warning: No ROOT found in {output_path}. Vina may fail.")
